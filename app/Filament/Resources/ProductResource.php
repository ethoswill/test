                ...
                Tables\Actions\BulkActionGroup::make([
                    Tables\Actions\Action::make('view')
                        ...
                        ->action(function ($records) {
                            if ($records->count() === 1) {
                                return redirect()->route('filament.admin.resources.products.view', $records->first());
                            }
                            Notification::make()
                                ->title('Please select only one product to view')
                                ->warning()
                                ->send();
                        })
                    ...
                    Tables\Actions\Action::make('edit')
                        ...
                        ->action(function ($records) {
                            if ($records->count() === 1) {
                                return redirect()->route('filament.admin.resources.products.edit', $records->first());
                            }
                            Notification::make()
                                ->title('Please select only one product to edit')
                                ->warning()
                                ->send();
                        })
                        ->requiresConfirmation(false),
                    Tables\Actions\Action::make('duplicate')
                        ...
                        ->action(function ($records) {
                            if ($records->count() !== 1) {
                                Notification::make()
                                    ->title('Please select exactly one product to duplicate')
                                    ->warning()
                                    ->send();
                                return;
                            }
